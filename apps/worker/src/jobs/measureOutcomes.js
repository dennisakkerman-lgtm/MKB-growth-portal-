...
export async function measureOutcomes() {
    const actions = await query(`select a.id, a.account_id, a.published_url, a.published_at
       from actions a
    ...
    for (const a of actions) {
        const [before] = await query(`select coalesce(sum(clicks),0) as clicks,
              sum(position * impressions) / nullif(sum(impressions),0) as pos
         from gsc_page_daily
        where account_id = $1 and page = $2
          and date >= $3::date - 28 and date < $3::date`, [a.account_id, a.published_url, a.published_at]);
        const [after] = await query(`select coalesce(sum(clicks),0) as clicks,
              sum(position * impressions) / nullif(sum(impressions),0) as pos
         from gsc_page_daily
        where account_id = $1 and page = $2
          and date >= $3::date and date < $3::date + 28`, [a.account_id, a.published_url, a.published_at]);
        const [acct] = await query(`select avg_deal_value, conv_rate from accounts where id = $1`, [a.account_id]);
        ...
        const deltaValue = dealValue > 0 && convRate > 0
            ...
            : null;
        await query(`insert into action_outcomes
       ...
       on conflict (action_id) do update set
         ...
         measured_at  = now()`, [a.id, Number(before?.clicks ?? 0), before?.pos ? Number(before.pos) : null,
